...
void display() {
    ...
    float student_avg;
    system("cls");
    printf(" NAME\t\tQUIZ1\tQUIZ2\tQUIZ3\tAVERAGE\n");
    for (i = 0; i <= last; i++) {
...
void download() {
    ...
    if (fp == NULL) {
        ...
        while (!feof(fp)) {
            if (fscanf(fp, " %[^\n]\n%d\n%d\n%d\n%d\n", nm, &a, &quiz1, &quiz2, &quiz3) == 5) {
                last++;
                strcpy(name[last], nm);
                age[last] = a;
                q1[last] = quiz1;
                q2[last] = q2[last] = quiz2;
                q3[last] = quiz3;
            }
        }
        fclose(fp);
...
void update_record() {
    char searchName[31];
    int i, d, found = 0, new_q1, new_q2, new_q3, choice;
    float avg;
    ...
    for (i = 0; i <= last; i++) {
        if (strcmp(name[i], searchName) == 0) {
            ...
            while (choice != 4) {
                ...
                switch(choice) {
                    case 1: printf("\nEnter new Quiz 1 score: ");scanf("%d", &new_q1);q1[i] = new_q1;
                        printf("Quiz 1 score updated successfully.\n");return;
                        break;
                    case 2: printf("\nEnter new Quiz 2 score: ");scanf("%d", &new_q2);q2[i] = new_q2;
                        printf("Quiz 2 score updated successfully.\n");return;
                        break;
                    case 3: printf("\nEnter new Quiz 3 score: ");scanf("%d", &new_q3);q3[i] = new_q3;
